...
def igm_transmission(wavelength, redshift, lines_width):
    ...
    # For Lyman alpha before end of reionization, f_trans(z) = 0.2 - 0.01*z
    # from Dijkstra & Wyithe (2010, MNRAS 408, 352)
    if redshift > 7:
        line_Lya = 121.6*(1+redshift)
        width_Lya = line_Lya * lines_width / cst.c * (1+redshift)
        w_Lya = np.logical_and(wavelength > line_Lya - 3.*width_Lya,
                               wavelength < line_Lya + 3.*width_Lya)
        smooth = np.ones_like(weight[w_Lya])
        #print('smooth', smooth)
        for i, j in enumerate(smooth):
            smooth[i] = ((0.2 - 0.01 * redshift) +
                         0.5*(1+erf(float(i)-0.75*len(smooth)))*
                             (1-(0.2 - 0.01 * redshift)))
        #print('smooth', smooth, (0.2 - 0.01 * redshift))

        weight[w_Lya] *= smooth
        #print('w_Lya 1', weight[w_Lya])
        #weight[w_Lya] *= 0.1*(1+np.erf(x-2) - 0.01 * redshift
...
class igm(SedModule):
    ...
    It also redshifts a rest-frame SED. If the SED is already redshifted,
    ...
    parameter_list = OrderedDict([
        ("igm", (
            "string()",
            "Method used for the IGM."
            "The only one at the moment is Meiksin (2006)",
            "Meiksin"
        ))
    ])

    def _init_code(self):
        """Attenuation in the IGM
        """

        self.igm_method = self.parameters["igm"]

        # Raise an error if Meiksin is not the selected method.
        if self.igm_method !=  "Meiksin":
            raise Exception("The only one at the moment is Meiksin (2006)")
    ...
    def process(self, sed):
        """Apply the IGM attenuation to the SED
        ...
        """
        self.redshift = sed.info['universe.redshift']
        self.universe_age = sed.info['universe.age']
        self.luminosity_distance = sed.info['universe.luminosity_distance']
        ...
        lines_width = sed.info['nebular.lines_width'] * 1e3
